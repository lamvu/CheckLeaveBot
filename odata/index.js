...
var ip2 = "http://acclimecc6.dev.acclim.local:8000/";
...
function get2(query) {
	return new Promise(function(resolve, reject) {
		request.get(ip2 + query).auth("acclim_app", "Acclim@7").end(function(err, result){
			if (err) {
				var res = err.response;
				var mes1;
				var mes2;
...
function post(query, data) {
	return new Promise(function(resolve, reject) {
		request.post(ip + query).set('Cookie', 'cookie_info').set('x-csrf-token', token).auth(uname, pword).send(data).end(function(err, result){
			if (err) {
				var res = err.response;
				var mes1;
				var mes2;
